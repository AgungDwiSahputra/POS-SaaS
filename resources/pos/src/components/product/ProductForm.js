...
const ProductForm = (props) => {
    ...
    const validateVariationTypesData = () => {
        ...
        const codeFrequency = variationTypesData.reduce((acc, variationType) => {
            acc[variationType.product_variation_code] = (acc[variationType.product_variation_code] || 0) + 1;
            return acc;
        }, {});
    ...
    const prepareFormData = () => {
        ...
        if (productValue.expiry_date) {
            formData.append("expiry_date", moment(productValue.expiry_date).format("YYYY-MM-DD"));
        }
